...
//! lives here: mosd MAPS an intent and writes the presence assertion, apid
...
/// it.
pub const DEFAULT_DECLARATION_PATH: &str = "/usr/lib/mos/recovery-actions.conf";
...
///
/// **One constant, two daemons.** mosd WRITES it after mapping an intent
...
/// operator made it on.
pub const DEFAULT_PRESENCE_MARKER_PATH: &str = "/run/mos/presence";
...
/// channel and a deadline. Declared once and serialized by mosd, deserialized
    ...
    fn every_malformed_declaration_maps_no_intent() {
        let cases: &[(&str, &str)] = &[
            ...
            (
                ...
                &ONE_ACTION.replace("_CHANNEL=/dev/tty0", "_CHANNEL=/var/lib/mos/console"),
            ),
            (
