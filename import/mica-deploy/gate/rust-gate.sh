...
# where the checkout lives and the gate cannot fix what it found; the image
# records what it is and the log reads that back, so the result names the
...
echo "=== rust gate: mica-deploy in ${IMAGE} ==="
docker run --rm \
    --label ai-agent=true \
