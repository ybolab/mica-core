...
/// A fake `com.mos.*` service: it answers `com.mos.Item1.GetItems` with
...
/// A fake that owns a `com.mos.*` name and answers something that is NOT
/// `com.mos.Item1` — the common shape of a service that is on the bus for its
...
/// A private bus, a `mosd` on it, and a client connection to drive it with.
struct Harness {
    address: String,
...
impl Harness {
    /// Start a private session bus and a `mosd` with the service scan running,
    ...
    async fn start_with_scan(scan: bool) -> Self {
        ...
        let mut command = Command::new(env!("CARGO_BIN_EXE_mosd"));
        command
        ...
        let mut mosd_child = command.spawn().expect("spawn mosd");
        ...
        let mosd_log = mosd_child.stdout.take().expect("piped stdout");
        std::thread::spawn(move || {
            let mut reader = BufReader::new(mosd_log);
        ...
        let mosd = ChildGuard(mosd_child);
        ...
        .await
        .expect("mosd never claimed com.mos.mosd on the private bus");
    ...
    /// `com.mos.Item1`, so the probe is refused rather than ignored.
    ...
    async fn wait_for_log(&self, fragments: &[&str]) -> String {
        ...
        loop {
            ...
            assert!(
                Instant::now() < deadline,
                "timed out waiting for {fragments:?} in mosd's log; the log was:\n{log}"
            );
            tokio::time::sleep(Duration::from_millis(25)).await;
        }
    }
...
async fn a_conforming_service_appears_with_no_conformance_gaps() {
    ...
    // mosd keeps its own name out of the registry: the registry is its view of
    // the other services on the bus.
    assert!(
        !has(&services, "com.mos.mosd"),
        "mosd must not register itself; got {services:#}"
    );
}
...
async fn a_vanished_service_is_retained_and_only_then_forgettable() {
    ...
    let start = xml
        .find(opening)
        .unwrap_or_else(|| panic!("no ForgetService on com.mos.mosd1:\n{xml}"));
...
/// 3. A service that answers no `com.mos.Item1` is still registered — warned
...
async fn a_service_without_item1_is_registered_and_warned_about() {
    ...
    for name in [silent, other] {
        ...
        assert_eq!(
            ...
            "a service answering no com.mos.Item1 must be published saying so — that fact IS \
