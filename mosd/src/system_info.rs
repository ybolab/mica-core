...
//! - `/proc/uptime`.
//! - `/usr/share/mos/meta/`, the baked half of the `meta/` seam
...
pub const BAKED_META_MANIFEST_PATH: &str = "usr/share/mos/meta/updates/manifest.json";
/// The development-grade marker, baked beside it exactly when the signing
/// material this image was built from was generated by
/// `pkgs/rauc/gen-dev-keys.sh`. Relative to the observer's root.
pub const BAKED_META_MARKER_PATH: &str = "usr/share/mos/meta/GENERATED";
...
/// The domains a `meta/GENERATED` marker names, from its `DOMAINS=` line.
///
/// The LAST such line wins, which is `gen-dev-keys.sh`'s own rule when it
/// merges a second domain into an existing marker. An absent or empty line is
/// an empty list rather than an error: the marker's presence is the claim, and
/// a marker that names no domain is still a marker.
fn marker_domains(text: &str) -> Vec<String> {
    text.lines()
        .filter_map(|line| line.strip_prefix(&format!("{MARKER_DOMAINS_KEY}=")))
        .next_back()
...
mod tests {
    ...
    /// because the mixed tree is real: a production RAUC ceremony's output
    ...
    async fn a_baked_marker_reports_development_and_the_domains_it_names() {
        ...
        std::fs::write(
            ...
            "DEVELOPMENT-GRADE, generated by pkgs/rauc/gen-dev-keys.sh.\n\nDOMAINS=rauc updates\n",
        )
        .expect("write");
        let evidence = HostSystemInfo::at(root.path())
            .observe()
        ...
        assert_eq!(
            evidence.trust,
            Some(TrustGrade::Development {
                domains: vec!["rauc".to_string(), "updates".to_string()],
            })
        );
        let info = info_json(&evidence, None, &daemon());
        assert_eq!(info["trust"]["grade"], "development");
        assert_eq!(info["trust"]["developmentDomains"][1], "updates");
        assert_eq!(
            info["trust"]["marker"],
    ...
    fn a_marker_naming_no_domain_is_still_development() {
        ...
        // The LAST line wins, which is the generator's own merge rule.
        assert_eq!(
            marker_domains("DOMAINS=rauc\nDOMAINS=rauc updates\n"),
            vec!["rauc".to_string(), "updates".to_string()]
        );
    }
}
