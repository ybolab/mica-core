...
//! bind mount that does not survive into that namespace. The file is mosd's to
...
/// so mosd can write it; the read-only root underneath cannot be written by
/// anyone.
...
impl HostnameExecutor for Hostnamed {
    ...
    async fn set_static_hostname(&self, name: &str) -> anyhow::Result<()> {
        ...
        // one path in mosd that is, and every other renderer needs the rename;
...
mod file_tests {
    ...
    async fn the_production_executor_writes_the_hostname_file() {
        ...
        assert_eq!(
            written, "edge-42\n",
            "the hostname file must hold exactly the name and a newline: mos-apply-hostname.service runs `hostname -F` on it at every boot"
        );
    }
    ...
    async fn the_hostname_file_holds_exactly_the_name() {
        ...
        assert_eq!(
            ...
            "edge-42\n",
            "mos-apply-hostname.service runs `hostname -F` on this file at every boot"
        );
    }
}
