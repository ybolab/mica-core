...
/// The `a{sa{sv}}` body of `com.mos.Item1.GetItems`: item path -> attributes.
type Items = HashMap<String, HashMap<String, OwnedValue>>;

/// What one probe of a service established.
struct Probe {
    /// The service answered `com.mos.Item1.GetItems` at the root.
    item1: bool,
...
struct Conformance {
    /// No `com.mos.Item1` answered at the service's root object.
    no_item1: bool,
...
///
/// Owns its strings on purpose: [`mos_busname::BusName`] borrows from the name
...
/// Every `com.mos.*` service mosd has seen, keyed by bus name.
///
...
async fn probe(connection: &Connection, name: &str) -> Probe {
    ...
    let items = match tokio::time::timeout(PROBE_TIMEOUT, call).await {
        ...
        Ok(Err(err)) => {
            tracing::debug!(service = name, error = %err, "service answers no com.mos.Item1");
            None
        }
        Err(_) => {
            tracing::debug!(
...
async fn record_appearance(
    ...
    // Never mosd itself: the registry is mosd's view of the OTHER services on
    // the bus, and probing our own name would have the daemon call into its
    // own object server and wait for itself.
    if name == BUS_NAME {
        return;
    }
    // The one rule, taken rather than restated (`mos-busname`). A name that
    // does not parse is not one of ours — `com.mos` itself reaches here,
    // because `arg0namespace='com.mos'` matches the namespace as well as the
    // names under it.
    let Some(parsed) = mos_busname::parse(name) else {
...
/// every name that is not ours and mosd is never woken to discard it here.
///
/// The namespace is [`mos_busname::PREFIX`] without its trailing dot rather
...
/// draw, so `com.mos.extra` is matched (it is ours) and `com.mosquitto` is
/// not.
...
mod tests {
    ...
    fn the_bus_filters_on_the_namespace() {
        let rule = name_owner_changed_rule()
            ...
            .to_string();
        assert!(
            rule.contains("arg0namespace='com.mos'"),
