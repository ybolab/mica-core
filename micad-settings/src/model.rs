...
/// broker nor the bridge runs: no `mica-mqtt-broker.service`, no
/// `mica-mqttd.service`. It validates nothing and depends on nothing below it —
...
/// `/var/lib/mica/mqtt-broker-users.toml` on STATE instead, which is how the
...
/// `hash` sits on apid's redaction denylist -- micad answers a settings read
...
/// by another (micad's `reset` module) on the next boot, replaying the same
...
/// which door was used; nothing re-derives permission from it, and micad does
...
///
/// **This is the one statement of the predicate.** `micad`'s station renderer
...
/// bound used to live inside `micad`'s private `encode_psk`, so the one crate
...
/// (`micad/src/reconciler/network.rs`, the security boundary for a settings
