...
//! `ring::digest`, `toml`, `micad_settings` and [`crate::identity`]. No socket,
...
//!   from [`micad_settings::validate_wifi_psk`], which deliberately names
...
//! - the import record ([`micad_settings::ProvisioningImport`]) holds a source,
...
/// `micad_settings` to reuse, so the document injects the identity and lets
