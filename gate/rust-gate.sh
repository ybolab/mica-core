...
for p in "${REPO_ROOT}/Cargo.toml" "${REPO_ROOT}/build-env/from.sh"; do
    [ -e "${p}" ] || {
        echo "error: ${p} does not exist. gate/rust-gate.sh derives the repository as one level above itself; build-env/ is the mica-build-env source pin (make deps)" >&2
        exit 1
    }
done
[ -x "${REPO_ROOT}/hack/check.sh" ] || { echo "error: ${REPO_ROOT}/hack/check.sh is missing or not executable; this gate runs that script and defines no gate of its own" >&2; exit 1; }
...
bash "${REPO_ROOT}/apid/ui/build.sh"
...
docker run --rm \
    ...
    "${IMAGE}" -c '
        ...
        echo "gate: micad with rustc ${MOS_BUILD_RUSTC}, clippy ${MOS_BUILD_CLIPPY}, rustfmt ${MOS_BUILD_RUSTFMT}, nextest ${MOS_BUILD_NEXTEST}, deny ${MOS_BUILD_DENY} from ${MOS_BUILD_IMAGE}"
        bash hack/check.sh
    '
