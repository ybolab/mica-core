...
import { InlineAlert } from '@/shared/components/product-layout'
...
export function TaskProgress({ taskId }: { taskId?: string }) {
  ...
  if (task.error) return <InlineAlert tone="warning" title={t('task.unconfirmed')} />
  ...
  return <InlineAlert tone={failed ? 'danger' : complete ? 'success' : 'warning'} title={message} />
}
