...
function TokenPanel() {
  ...
  return (
    <Card>
      ...
      <div className="collection-list">
        {tokens.data?.map((token) => (
          <div className="collection-row" key={token.id}>
            ...
            <Button variant="ghost" size="sm" aria-label={t('access.tokens.revokeLabel', { name: token.name })} onClick={() => revoke.mutate(token.id)}><Trash2 className="size-4" /></Button>
          </div>
        ))}
        {tokens.data?.length === 0 ? <p className="empty">{t('access.tokens.empty')}</p> : null}
      </div>
      ...
      {mint.error || revoke.error ? <p className="callout error" role="alert">{errorMessage(mint.error ?? revoke.error, t('common.requestFailed'))}</p> : null}
    </Card>
  )
}
...
function SshPanel() {
  ...
  const transientSubmit = (event: FormEvent) => {
    event.preventDefault()
    if (window.confirm(t('access.ssh.confirmTransient'))) transient.mutate()
  }
  ...
  return (
    <Card>
      ...
      <div className="service-state"><Status ok={enabled.data === true}>{t('access.ssh.status', { state: t(enabled.data ? 'common.states.enabled' : 'common.states.disabled') })}</Status><Button variant="secondary" size="sm" onClick={() => toggle.mutate(!enabled.data)}>{t(enabled.data ? 'common.actions.disable' : 'common.actions.enable')}</Button></div>
      ...
      <div className="collection-list">
        {keys.data?.keys.map((entry) => (
          <div className="collection-row" key={entry.fingerprint ?? entry.key}>
            ...
            {entry.fingerprint ? <Button variant="ghost" size="sm" aria-label={t('access.ssh.removeLabel')} onClick={() => remove.mutate(entry.fingerprint!)}><Trash2 className="size-4" /></Button> : null}
          </div>
        ))}
      </div>
      ...
      {toggle.error || add.error || remove.error ? <p className="callout error" role="alert">{errorMessage(toggle.error ?? add.error ?? remove.error, t('common.requestFailed'))}</p> : null}
